...
public class Startbildschirm
{
    ...
    public static void starteSpiel()
    {
        ...
        while(true)
        {
            ...
            if (eingabe != null)
            {
                ...
                if (eingabe.getKind() == Key.Kind.Escape) System.exit(0);
            }
        }
        ...
        y_offset =  1;
        terminal.moveCursor(x_offset, y_offset);
        for (char temp: "Hochschule".toCharArray()) terminal.putCharacter(temp);
        y_offset++;
        
        terminal.moveCursor(x_offset, y_offset);
        for (char temp: "fuer Technik".toCharArray()) terminal.putCharacter(temp);
        y_offset++;
        
        terminal.moveCursor(x_offset, y_offset);
        for (char temp: "Stuttgart".toCharArray()) terminal.putCharacter(temp);
        ...
        y_offset =  7;
        terminal.moveCursor(x_offset, y_offset);
        for (char temp: "Informatik/Mathematik".toCharArray()) terminal.putCharacter(temp);
        
        y_offset++;
        terminal.moveCursor(x_offset, y_offset);
        for (char temp: "Semester 1+ Vorschaltsemester".toCharArray()) terminal.putCharacter(temp);
        
        y_offset += 3;
        terminal.moveCursor(x_offset, y_offset);
        for (char temp: "kostenlos herunterladen / free download".toCharArray()) terminal.putCharacter(temp);
        
        y_offset++;
        terminal.moveCursor(x_offset, y_offset);
        terminal.applyForegroundColor(0, 113, 218);
        for (char temp: "github.com/9Lukas5/de_HFTStuttgart_WS16_Tetris/releases".toCharArray()) terminal.putCharacter(temp);
    ...
    public static void zeichne()
    {
        ...
        try
        {
            ...
            for (char temp: version.toCharArray()) terminal.putCharacter(temp);
    ...
    public static void ladeBildschirm()
    {
        ...
        try
        {
            ...
            for (int i=0; i < 20; i++)
            {
                if (i==0) terminal.applyForegroundColor(255, 0, 0);
                if (i==7) terminal.applyForegroundColor(0, 255, 0);
                if (i==14) terminal.applyForegroundColor(0, 0, 255);
    ...
    public static void initLogo()
	{
        ...
        for (int i=0; i < 18; i++) logo.add(temp.clone());
    ...
    public static void logoKlein()
    {
        ...
        for (int i=0; i < 5; i++) logo_Klein.add(temp.clone());

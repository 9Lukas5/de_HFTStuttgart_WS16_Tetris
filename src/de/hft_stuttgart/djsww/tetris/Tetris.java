...
public class Tetris
{
    ...
    public static void main(String[] args)
    {
        ...
        ingame: while(true)
        {
            ...
            {
                if (eingabe.getKind() == Key.Kind.NormalKey) checkHidden(eingabe);
                
                if (eingabe.getKind() == Key.Kind.ArrowLeft) naechster_StartPkt.x--;
                
                if (eingabe.getKind() == Key.Kind.ArrowRight) naechster_StartPkt.x++;
                
                if (eingabe.getKind() == Key.Kind.ArrowDown) naechster_StartPkt.y++;
                
				if (Character.toUpperCase(eingabe.getCharacter()) == 'P') pause();
                ...
                if (eingabe.getKind() == Key.Kind.Escape) break ingame;
            }
            
            if (zyklus % verzoegerung == 0) naechster_StartPkt.y++;
    ...
    public static int pruefeNeuePos(Punkt neuer_Startpkt)
    {
        ...
        for (Punkt feld: neue_Koord)
        {
            ...
            if (spielfeld.spielfeld.get(feld.y)[feld.x].gesperrt)
            {
                ...
                else return -2; // von der Seite beim drehen auf gesperrtes
            }                   // Feld aufgesessen
        }
        return 0;
    }
    ...
    {
        if (Character.toUpperCase(eingabe.getCharacter()) == 'I') next_Stein = new Tetromino_I();
        if (Character.toUpperCase(eingabe.getCharacter()) == 'J') next_Stein = new Tetromino_J();
        if (Character.toUpperCase(eingabe.getCharacter()) == 'L') next_Stein = new Tetromino_L();
        if (Character.toUpperCase(eingabe.getCharacter()) == 'O') next_Stein = new Tetromino_O();
        if (Character.toUpperCase(eingabe.getCharacter()) == 'S') next_Stein = new Tetromino_S();
        if (Character.toUpperCase(eingabe.getCharacter()) == 'T') next_Stein = new Tetromino_T();
        if (Character.toUpperCase(eingabe.getCharacter()) == 'Z') next_Stein = new Tetromino_Z();
	...
	public static void pause()
	{
		...
		while(true)
		{
			...
			if (eingabe != null && Character.toUpperCase(eingabe.getCharacter()) == 'P') break;
		}
